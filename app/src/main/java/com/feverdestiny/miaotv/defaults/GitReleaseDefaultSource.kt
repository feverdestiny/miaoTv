...
/**
 * 应用内更新的获取 / 下载策略：
 * - 检查：仅当地址是内置 GitHub Latest API（或其 gh-proxy）时，按 gh-proxy → 官方 API 回退。
...
object GitReleaseDefaultSource {
    fun isBuiltinLatestApi(sourceUrl: String): Boolean {
        ...
        return AppBuiltinEndpoints.GIT_RELEASE_LATEST_API_FALLBACKS.any {
            it.equals(trimmed, ignoreCase = true)
        }
    }

    /**
     * 检查更新的实际拉取顺序。内置 Latest API 始终从 gh-proxy 起试，
     * 避免把已失效的首个 host 当作唯一地址反复请求。
     */
