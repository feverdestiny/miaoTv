...
在全屏 **或分屏播放**时，**长按遥控器确认键**，可打开 **默认直播源快速切换**：在 **当前使用的订阅地址** 与 **设置里曾保存过的直播源历史** 之间选择并切换（仅切换、不添加新地址；历史至少再有一条时才会出现可选列表）。适合在多个 M3U / 订阅之间临时换源而无需先进入设置。

---

## 首次配置说明

- **直播源**：新安装默认使用本仓库 [`sources/cn.m3u`](https://cdn.jsdelivr.net/gh/feverdestiny/miaoTv@main/sources/cn.m3u)（中国频道聚合）。国内网络依次尝试 [jsDelivr](https://cdn.jsdelivr.net/gh/feverdestiny/miaoTv@main/sources/cn.m3u) → [gh-proxy](https://gh-proxy.com/https://raw.githubusercontent.com/feverdestiny/miaoTv/main/sources/cn.m3u) → [GitHub raw](https://raw.githubusercontent.com/feverdestiny/miaoTv/main/sources/cn.m3u)；jsDelivr 可能缓存数小时。仅看央视可在 **设置 → 直播源** 改为 [`sources/cctv.m3u`](https://cdn.jsdelivr.net/gh/feverdestiny/miaoTv@main/sources/cctv.m3u)（同样 jsDelivr → gh-proxy → GitHub raw，不与 `cn.m3u` 混用）。也可在设置或 **网页配置页** 填写你自己的 M3U / tvbox 风格 TXT 等地址（自定义地址不会走上述回退）。
...
- **影视仓/TVBox 风格直播源**
  ...
  - 支持单频道多线路（`地址1#地址2`）与分辨率后缀（如 `地址$1920×1080`，会自动忽略 `$` 后描述）。
  - 简例：
    - `央视频道,#genre#`
